<template>
    <div>
        <div class="scoreNav">
            <div @click="selectScore(index)" :class="selectScoreNav == index ? 'item select' : 'item'" v-for="(item,index) in scoreNav" :key="'score'+index">123{{item}}</div>
        </div>
    ...
    methods:{
        ...
        selectScore(index){
            ...
            if(index == 0){
                ...
                this.$router.push('/score/termReview')
            }
        },
        selectNav(index){
            this.selectLi = index
